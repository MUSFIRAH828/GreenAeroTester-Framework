...
def generate_chart(ranked_df: pd.DataFrame):
    """
    Two-panel bar chart:
        Left  — average energy (Wh) per test, coloured by mandatory / optional.
        Right — utility score per test with priority rank annotated on each bar.
    ...
    fig.suptitle(
        "GreenAeroTest Prototype  —  Energy & Priority Overview\n"
        "(values represent PC simulation energy, not aircraft fuel)",
